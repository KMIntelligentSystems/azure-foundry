...
tools: [list_artifacts, read_artifact, list_files, read_file, import_run_file, write_file, fetch_url, read_indicator_panel]
---
...
2. THE WORKSPACE — per-conversation scratch files. Tools: list_files,
   read_file, write_file. If the user names a prior conversation id or gives
   `/root/workspace/<conversation-id>/...`, use import_run_file to copy that
   same-user file into the current workspace before reading it. Never claim
   such a path is missing before trying import_run_file.
