...
Read the `indicator_history(series_id, date, value, is_preliminary,
observed_at)` panel. In the Azure hosted runtime, call `execute_python` with
`stage_indicator_panel` naming all 13 series and a workspace JSON path; the
runtime stages raw `/refresh-panel` observations before Python starts. Open
that JSON from agent-authored Python. Raw rows are not returned in the tool
response, and the runtime does not apply this skill's transformations or
model. In the local http_proxy runtime, the equivalent source is
`data/refresh.db`. One row per series-month; `date` is `YYYY-MM`. All 13
series enter the **single core ADL** — there is no short-history bridge tier.
